...
[ %infected            ;; what % of the population is infectious
  ...
  carrying-capacity    ;; the number of organisms that can be in the world at one time
  %dead                ;; what % of the original population is dead
  num-dead             ;; number of organisms that have died
  num-vaccinated       ;; number of initial population that have been vaccinated
  infectiousness
  chance-recover
  duration
  reproduction-chance
...
;; We create a variable number of organisms of which 10 are infectious,
;; and distribute them randomly
to setup-organisms
  create-organisms number-organisms
  [ setxy random-xcor random-ycor
    set age random max-age
    set size 1.5  ;; easier to see
    get-healthy ]
  ask up-to-n-of num-vaccinated organisms
    [ become-immune ]
  ask up-to-n-of initial-sick organisms with [remaining-immunity = 0]
    [ get-sick ]
end

to get-sick ;; organism procedure
  set sick? true
  set remaining-immunity 0
end

to get-healthy ;; organism procedure
  set sick? false
  set remaining-immunity 0
  set sick-time 0
end

to become-immune ;; organism procedure
  set sick? false
  set sick-time 0
  set remaining-immunity immunity-duration * 360
end

;; This sets up basic constants of the model.
to setup-constants
  set lifespan 50 * 360      ;; 50 times 360 days
...
to update-global-variables
  if count organisms > 0
    [ set %infected (count organisms with [ sick? ] / count organisms) * 100
      set %immune (count organisms with [ immune? ] / count organisms) * 100
  ]
  set num-vaccinated 0
  set infectiousness 99
  set chance-recover 75
  set duration 10
  (ifelse
    turtle-shape = "person" [
    set reproduction-chance 1
    set max-age 85
    ]
    turtle-shape = "cow" [
      set reproduction-chance 10
      set max-age 20
    ]
    turtle-shape = "star" [
      set reproduction-chance 20
      set max-age 35
    ]
    turtle-shape = "bird side" [
      set reproduction-chance 20
      set max-age 10
    ]
    [
  ...
  set transmission-type "touch"
  set vector-infection false
  set indirect-infection false
end
...
;;organism counting variables are advanced.
to get-older ;; organism procedure
             ;; organisms die of old age once their age exceeds the
             ;; lifespan (set at 50 years in this model).
  set age age + 1
  if age > max-age * 360 [ die ]
  if immune? and not immunity-forever? [ set remaining-immunity remaining-immunity - 1 ]
  if sick? [ set sick-time sick-time + 1 ]
end

;; organisms move about at random.
to move ;; organism procedure
  rt random 100
  lt random 100
  fd 1
end

;; If a organism is sick, it infects other organisms on the same patch.
;; Immune organisms don't get sick.
to infect ;; organism procedure
  if transmission-type = "touch"
  [
    ask other organisms in-radius 0.5 with [ not sick? and not immune? ]
    [ if random-float 100 < infectiousness
      [ get-sick ] ]
  ]
  if transmission-type = "airborne"
  [
    ask other organisms in-radius 1.5 with [ not sick? and not immune? ]
    [ if random-float 100 < infectiousness
      [ get-sick ] ]
  ]
end
...
to vector-commands
  ifelse vector-infection = true [
    create-vectors (100 - count vectors) [             ;;vector population is capped at 100
      set shape "bug"
      setxy random-xcor random-ycor
      set size 1
      set sick? false
    ...
    ask vectors [
      ...
      fd 1
      set age age + 1
      if age = 60 [die]                                 ;;vectors die after 60 days of living
      if any? organisms in-radius 1 with [sick?]        ;;vector carries disease if it feeds on sick organsims
      [
        if random-float 100 < infectiousness
          [ set sick? true
            set color blue
      ...
      if sick? [                                        ;;if vector is carrying disease, vector infects healthey organisms
        ask organisms in-radius 0.5 with [ not sick? and not immune? ]
        [ if random-float 100 < infectiousness
          [ get-sick ] ]
      ]
    ]
...
to indirect-commands
  ifelse indirect-infection = true [
    ask ponds
    [
      if any? organisms in-radius 1 with [sick?]     ;;pond carries disease if sick organsims drinks from it
      [
        if random-float 100 < infectiousness
          [ set sick? true
            set color red
      ...
      if sick? [                                     ;;if pond is carrying disease, pond infects healthey organisms
        ask organisms in-radius 1 with [ not sick? and not immune? ]
        [ if random-float 100 < infectiousness
          [ get-sick ] ]
        set sick-time sick-time + 1
      ]
      if sick-time > 60 [                            ;;pond no longer carries the disease after 60 days
        set sick? false
        set color blue
      ]
    ]
...
;; Once the organism has been sick long enough, it
;; either recovers (and becomes immune) or it dies.
to recover-or-die ;; organism procedure
  if sick-time > duration                        ;; If the organism has survived past the virus' duration, then
  ...
  set %dead (num-dead / number-organisms) * 100
end

;; If there are less organisms than the carrying-capacity
;; then organisms can reproduce.
to reproduce
  if count organisms < carrying-capacity and random-float 100 < reproduction-chance
    [ hatch 1
      [ set age 1
        lt 45 fd 1
        get-healthy ] ]
...
turtle-shape
turtle-shape
"person" "circle" "star" "cow" "bird side"
3
...
258
Make One Sick
ask up-to-n-of 1 turtles with [remaining-immunity = 0 and not sick?] [get-sick]
NIL
1
T
OBSERVER
...
The model is initialized with 300 people, of which 90 are infected.  People move randomly about the world in one of three states: healthy but susceptible to infection (green), sick and infectious (red), and healthy and immune (gray). People may die of infection or old age.  When the population dips below the environment's "carrying capacity" (set at 300 in this model) healthy people may produce healthy (but susceptible) offspring.

Some of these factors are summarized below with an explanation of how each one is treated in this model.

### The density of the population

Population density affects how often infected, immune and susceptible individuals come into contact with each other. You can change the size of the initial population through the NUMBER-ORGANISMS slider.

### Population turnover

As individuals die, some who die will be infected, some will be susceptible and some will be immune.  All the new individuals who are born, replacing those who die, will be susceptible.  People may die from the virus or they may die of old age.
...
How easily does the virus spread?  Some viruses with which we are familiar spread very easily.  Some viruses spread from the smallest contact every time.  Others (the HIV virus, which is responsible for AIDS, for example) require significant contact, perhaps many times, before the virus is transmitted.

### Duration of infectiousness

How long is a person infected before they either recover or die?  This length of time is essentially the virus's window of opportunity for transmission to new hosts.
...
The IMMUNITY-DURATION slider determines how many years an organism is immune after getting sick and recovering. This can be set to an indefinite amount of time with the IMMUNITY-FOREVER? checkbox.
...
true
10
Circle -13345367 true true 42 42 216
Circle -13345367 true true 11 41 127
Circle -13345367 true true 41 26 127
Circle -13345367 true true 146 71 127
Circle -13345367 true true 146 101 127
Circle -13345367 true true 71 146 127
